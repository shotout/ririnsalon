...
Class Users extends DBConnection {
	...
	public function save_users(){
		...
		foreach($_POST as $k => $v){
			if(in_array($k,array('nama','username','type'))){
				if(!empty($data)) $data .=" , ";
				$data .= " {$k} = '{$v}' ";
			}
		}
		...
		if(empty($id)){
			...
			if($qry){
				...
				$this->settings->set_flashdata('success','Data Admin berhasil disimpan.');
				$resp['status'] = 1;
			}else{
			...
			if($qry){
				$this->settings->set_flashdata('success','Data Admin berhasil diperbarui.');
		...
		if($resp['status'] == 1){
			$data="";
			foreach($_POST as $k => $v){
				if(!in_array($k,array('id','nama','username','password','type','oldpassword'))){
					if(!empty($data)) $data .=", ";
	...
	public function delete_users(){
		...
		if($qry){
			...
			$this->settings->set_flashdata('success','Data Admin berhasil dihapus.');
